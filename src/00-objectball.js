function gameObject() {
    return {
        home: {
            teamName: "Brooklyn Nets",
            colors: ["Black", "White"],
            players: {
                "Alan Anderson": {
                    "number": 0,
                    "shoe": 16,
                    "points": 22,
                    "rebounds": 12,
                    "assists": 12,
                    "steals": 3,
                    "blocks": 1,
                    "slamDunks": 1
                },
                "Reggie Evans": {
                    "number": 30,
                    "shoe": 14,
                    "points": 12,
                    "rebounds": 12,
                    "assists": 12,
                    "steals": 12,
                    "blocks": 12,
                    "slamDunks": 7
                },
                "Brook Lopez": {
                    "number": 11,
                    "shoe": 17,
                    "points": 17,
                    "rebounds": 19,
                    "assists": 10,
                    "steals": 3,
                    "blocks": 1,
                    "slamDunks": 15
                },
                "Mason Plumlee": {
                    "number": 1,
                    "shoe": 19,
                    "points": 26,
                    "rebounds": 12,
                    "assists": 6,
                    "steals": 3,
                    "blocks": 8,
                    "slamDunks": 5
                },
                "Jason Terry": {
                    "number": 31,
                    "shoe": 15,
                    "points": 19,
                    "rebounds": 2,
                    "assists": 2,
                    "steals": 4,
                    "blocks": 11,
                    "slamDunks": 1
                },
            },
        },
        away: {
            teamName: "Charlotte Hornets",
            colors: ["Turquoise", "Purple"],
            players: {
                "Jeff Adrien": {
                    "number": 4,
                    "shoe": 18,
                    "points": 10,
                    "rebounds": 1,
                    "assists": 1,
                    "steals": 2,
                    "blocks": 7,
                    "slamDunks": 2
                },
                "Bismak Biyombo": {
                    "number": 0,
                    "shoe": 16,
                    "points": 12,
                    "rebounds": 4,
                    "assists": 7,
                    "steals": 7,
                    "blocks": 15,
                    "slamDunks": 10
                },
                "DeSagna Diop": {
                    "number": 2,
                    "shoe": 14,
                    "points": 24,
                    "rebounds": 12,
                    "assists": 12,
                    "steals": 4,
                    "blocks": 5,
                    "slamDunks": 5
                },
                "Ben Gordon": {
                    "number": 8,
                    "shoe": 15,
                    "points": 33,
                    "rebounds": 3,
                    "assists": 2,
                    "steals": 1,
                    "blocks": 1,
                    "slamDunks": 0
                },
                "Brendan Haywood": {
                    "number": 33,
                    "shoe": 15,
                    "points": 6,
                    "rebounds": 12,
                    "assists": 12,
                    "steals": 22,
                    "blocks": 5,
                    "slamDunks": 12
                },
            }
        }
    }

};



function numPointsScored(playerName) {
    const gameData = gameObject();

    const homePlayer = gameData.home.players[playerName];
    const awayPlayer = gameData.away.players[playerName];

    if (homePlayer) {
        return homePlayer.points;
    }
    else if (awayPlayer) {
        return awayPlayer.points;
    }
    else {
        return "player not found";
    }
}

console.log(numPointsScored("Brendan Haywood"))

function shoeSize(playerName) {
    const gameData = gameObject();
    const homePlayer = gameData.home.players[playerName];
    const awayPlayer = gameData.away.players[playerName];

    if (homePlayer) {
        return homePlayer.shoe;
    }
    else if (awayPlayer) {
        return awayPlayer.shoe;
    }
    else {
        return "player shoe size not found";
    }
}

console.log(shoeSize("Reggie Evans"))

function teamColors(teamName) {
    const gameData = gameObject();
    const home = gameData.home.colors;
    const away = gameData.away.colors;

    if (home) {
        return home;
    }
    else if (away) {
        return away;
    }
    else {
        return "team not found";
    }
}

console.log(teamColors("Brooklyn Nets"))

function teamNames() {
    const gameData = gameObject();
    let home = gameData.home.teamName;
    let away = gameData.away.teamName;
    console.log(home)
    return [home, away]
}

console.log(teamNames(gameData));

function getPlayerNumbers() {
    const gameData = gameObject();
    const homePlayers = Object.values(gameData.home.players);
    const awayPlayers = Object.values(gameData.away.players);

    const allPlayers = [...homePlayers, ...awayPlayers];
    const PlayerNumbers = allPlayers.map(players => players.number);

    return PlayerNumbers;
}

console.log(getPlayerNumbers())


function playerStats(playerName) {
    const gameData = gameObject();
    const homePlayers = gameData.home.players;
    const awayPlayers = gameData.away.players;

    var res1 = homePlayers[playerName];
    var res2 = awayPlayers[playerName];

    if (res1 === null || res1 === undefined) {
        return res2;
    }
    else {
        return res1;
    }
};

console.log(playerStats("Brook Lopez"))


function bigShoeRebounds() {
    const gameData = gameObject();
    let largestShoSize = 0;
    let playerWithLargestShoe = null;
    let reboundsofPlayer = 0;

    for (const team in gameData) {
        for (const player in gameData[team].players) {
            const shoeSize = gameData[team].players[player].shoe;
            if (shoeSize > largestShoSize) {
                largestShoSize = shoeSize;
                playerWithLargestShoe = player;
                reboundsofPlayer = gameData[team].players[player].rebounds;
            }
        }
    }
    return {
        reboundsofPlayer
};
}
console.log(bigShoeRebounds())